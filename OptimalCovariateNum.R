...
cvrtNoSelect<-function(reaDir,writeDir,N){  # N is the number of PC
       ...
       sigfPeaks<-data.frame(cvrtnum=factor(rep(paste0(1:N,'PCno'),each=length(seq(0,0.25,0.01))),levels=paste0(1:N,'PCno')),sigfPeaksNum=significantQ,fdr=rep(seq(0,0.25,0.01),N))
