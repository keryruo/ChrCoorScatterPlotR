#### the original code was writtern by stephenturner ####
#### from https://github.com/stephenturner/qqman/blob/master/R/manhattan.R ####
...
manhattan <- function(x, chr="CHR", bp="BP", p="P", snp="SNP", 
   ...
   d$index = rep.int(seq_along(unique(d$CHR)), times = tapply(d$SNP,d$CHR,length))  # replcace the for loop of line 92-96 to improve efficiency
    ...
    par(mar=c(8,10,4.1,2.1))
    # The old way to initialize the plot
    # plot(NULL, xaxt='n', bty='l', xaxs='i', yaxs='i', xlim=c(xmin,xmax), ylim=c(ymin,ymax),
    #      xlab=xlabel, ylab=expression(-log[10](italic(FDR))), las=1, pch=20, ...)

    # The new way to initialize the plot.
    ## See http://stackoverflow.com/q/23922130/654296
    ## First, define your default arguments
    def_args <- list(xaxt='n', bty='l', xaxs='i', yaxs='i', las=1, pch=20,cex.lab=3,
                     xlim=c(xmin,xmax), ylim=c(0,ceiling(max(d$logp))),
                     xlab=xlabel, ylab=expression(-log[10](italic(FDR))))
    ...
    if (!is.null(annotatePval)) {
        ...
        par(xpd = TRUE,mar=c(8,10,4.1,2.1))
        # annotate these SNPs
